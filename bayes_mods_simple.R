...
model_binom_svl.sex.hab <- function() {
  ## Specify likelihood
  for(i in 1:length(x1)){
    #y[i] ~ dbin(p[i], 1)
    y[i] ~ dbin(p[i], 1)
...
model_beta_3 <- function() {
  ...
  for(i in 1:length(x1)){
    ...
    alpha[i] <- mu[i] * phi
    beta[i]  <- (1-mu[i]) * phi
    logit(mu[i]) <- b0 + b1*x1[i] + b2*x2[i] + b3*x1[i]*x2[i] + b4*x1[i]*x2[i] + b5*x1[i]*x3[i]
    LogLik[i] <- log(dbeta(y[i], alpha[i], beta[i]))
  }
  ## Specify priors
  phi ~ dgamma(0.1,0.1)
...
model_beta_3.4 <- function() {
  ...
  for(i in 1:length(x1)){
    ...
    alpha[i] <- mu[i] * phi
    beta[i]  <- (1-mu[i]) * phi
    logit(mu[i]) <- b0 + b1*x1[i] + b2*x2[i] + b3*x3[i] + b4*x4[i] + 
                          b5*x2[i]*x3[i] + b6*x2[i]*x4[i]
    LogLik[i] <- log(dbeta(y[i], alpha[i], beta[i]))
  }
  ## Specify priors
...
mod.infection.svl=jags(model.file=model_binom_svl,data=infection.svl,
                       n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
                       parameters.to.save=c("alpha","b1","LogLik"))

#p(infection)  ~ Sex
infection.sex <- data.frame(y = diagnosed$infection, x1 = diagnosed$SEX)
mod.infection.sex=jags(model.file=model_binom_sex,data=infection.sex,
                       n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
                       parameters.to.save=c("alpha","b1","LogLik"))
...
#p(infection)~(SVL*Sex)
infection.svl.sex <- data.frame(y = diagnosed$infection, 
                                x1 = diagnosed$SVL, x2 = diagnosed$SEX)
mod.infection.svl.sex=jags(model.file=model_binom_svl.sex,data=infection.svl.sex,
                           n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
...
mod.infection.svl.habitat=jags(model.file=model_binom_svl.hab,data=infection.svl.habitat,
                               n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
                               parameters.to.save=c("alpha","b1","b2","b3","LogLik"))
...
mod.infection.sex.habitat=jags(model.file=model_binom_sex.hab,data=infection.sex.habitat,
                               n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
                               parameters.to.save=c("alpha","b1","b2","LogLik"))


#p(infection)~(SVL*Sex)+(SVL*Habitat)
infection.svl.sex.habitat <- data.frame(y = diagnosed$infection,
                                        x1 = diagnosed$SVL, x2 = diagnosed$SEX, x3 = diagnosed$habitat)
mod.infection.svl.sex.habitat=jags(model.file=model_binom_svl.sex.hab,data=infection.svl.sex.habitat,
                                   n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,
